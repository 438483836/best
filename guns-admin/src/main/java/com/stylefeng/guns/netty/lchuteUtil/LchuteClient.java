...
public class LchuteClient {
    ...
    private static final Logger logger = LoggerFactory.getLogger(LchuteClient.class);

    public LchuteClient(ApplicationContext applicationContext){
    ...
    public LchuteCommand invokeSync(String addr, final LchuteCommand request, long timeoutMillis)
            throws InterruptedException, RemotingConnectException, RemotingSendRequestException,
            RemotingTimeoutException {
        final Channel channel = this.getAndCreateChannel(addr);
        if (channel != null && channel.isActive()) {
            try {
                LchuteCommand response = null;//this.invokeSyncImpl(channel, request, timeoutMillis);
                return response;
            } catch (Exception e) {
    ...
    public static void main(String[] args) throws Exception {
        final LchuteClient client = new LchuteClient(null);
        client.start();

        Channel channel = client.getAndCreateChannel("192.168.1.4:16002");

        try {
            LchuteCommand lchuteCommand = new LchuteCommand();
            lchuteCommand.setSupplyNo((short) 23);
            lchuteCommand.setLchute(3244);
            lchuteCommand.setYcbz((byte) 0);
            lchuteCommand.setLjgksjly((byte) 1);
            lchuteCommand.setScanno((byte) 2);
            lchuteCommand.setBarcodeS("1122334455667788990011");

            channel.writeAndFlush(lchuteCommand).addListener(new ChannelFutureListener() {
                @Override
                public void operationComplete(ChannelFuture f) throws Exception {
                    if (f.isSuccess()) {
                        System.out.println("SendRequestOK");
                        return;
                    }
                    else {
                        System.out.println("SendRequestFalse");
                    }
                }
            });
        } catch (Exception e) {
